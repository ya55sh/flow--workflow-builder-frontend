...
export default function SidePanel({ stepId, stepType }: SidePanelProps) {
  ...
  const stagedApp: StagedApp[] = useSelector(
    (state: any) => state.workflowApp.stagedApp
  );
  const selectedStaged: StagedApp | undefined = stagedApp.find(
    (app) => app.stepId === stepId
  );
  ...
  const currentStep: WorkflowStep | undefined = workflow.steps.find(
    (step: WorkflowStep) => step.id === stepId
  );

  // Find trigger step to determine available variables for conditions
  const triggerStep = workflow.steps.find(
    (step: WorkflowStep) => step.type === "trigger"
  );
  ...
  useEffect(() => {
    if (currentStep?.config) {
      ...
      if (
        selectedStaged?.appName === "webhook" &&
        configToSet.payload !== undefined
      ) {
  ...
  useEffect(() => {
    ...
    selectedStaged?.appName,
    selectedStaged?.hasUser,
    selectedStaged?.expired,
  ]);
  ...
  function handleAppSignin() {
    ...
    const appToConnect = userApps.find(
      (a: App) => a.appName === selectedStaged?.appName
    );
    ...
    if (appToConnect) {
      ...
      const handleMessage = (e: MessageEvent) => {
        if (e.origin !== "https://api.snare.co.in") return;
  ...
  function updateCondition(
    ...
    const updated = visualConditions.map((c) =>
  ...
  function saveVisualConditionsWithValues(
    ...
    const availableSteps =
      stepType === "condition"
        ? workflow.steps.filter(
            (step: WorkflowStep) => Number(step.id) > Number(stepId)
          )
      ...
      return stepType === "condition" ? (
        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
          ...
          <p className="text-xs text-gray-700">
            Add more steps after this condition using the ➕ button below.
          </p>
        </div>
      ) : null;
    }
      ...
      <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
        ...
        <div className="flex flex-wrap gap-2">
          {availableSteps.map((step: WorkflowStep) => (
            ...
            >
              <code className="text-indigo-600 font-bold text-xs">
                {step.id}
              </code>
          ...
          return (
            <div key={field.name} className="flex flex-col gap-1">
              ...
              <select
                value={value}
                onChange={(e) =>
                  handleDynamicConfigChange(field.name, e.target.value)
                }
            ...
            <div key={field.name} className="flex flex-col gap-1">
              ...
              {isNotConnected ? (
                <div className="p-2 border border-yellow-300 bg-yellow-50 rounded text-sm text-yellow-700">
                  Connect your account to load {field.label.toLowerCase()}{" "}
                  options
                </div>
              ) : (
                <select
          ...
          <div>
            ...
            <div className="p-2 border-1 border-gray-500/50 rounded-lg">
              {selectedStaged.appName === `google`
                ? `Gmail`
                : `${selectedStaged.appName[0].toUpperCase()}${selectedStaged.appName.substring(
                    1
                  )}`}
            </div>
          </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-lg border border-blue-200">
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">
                    Create rules to control workflow branching based on trigger
                    data.
                  </p>
                </div>
                            ...
                            {workflow.steps
                              ...
                              .map((s: WorkflowStep) => (
                                <option key={s.id} value={s.id}>
                                  Step {s.id} ({s.type}) -{" "}
                                  {s.title || s.appName || "Not configured"}
                                </option>
                              ))}
                          </select>
                          {workflow.steps.filter(
                            (s: WorkflowStep) => Number(s.id) > Number(stepId)
                          ).length === 0 && (
                            <p className="text-xs text-gray-600 mt-1">
                              ⚠️ No steps after this condition. Add more steps
                              below.
                            </p>
                          )}
                        </div>
